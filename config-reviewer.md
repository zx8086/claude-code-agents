...
6. **Enforce 4-pillar pattern compliance - reject any other patterns**
...
1. **Pillar 1: Default Configuration Object** - All baseline values in `defaultConfig`
2. **Pillar 2: Environment Variable Mapping** - Explicit `envVarMapping` with `as const`
3. **Pillar 3: Manual Configuration Loading** - `loadConfigFromEnv()` with fallbacks
4. **Pillar 4: Validation at End** - Pure Zod schema validation, NO defaults in schema
